...
- 服务端命令：`/htmlui <pageId>` 打开指定页面
- 模板变量：在 HTML 中用 `{player}` 等占位符，服务端自动替换
- ActionHandler：`ActionHandler.registerHandler("actionName", handler)` 注册自定义动作
- 主动下发：`HtmlLoader.sendTo(player, "pageId")` 强制推送页面
